...
export default {
  ...
  async mounted () {
    ...
    await axios.post(config.backend + "md/" + this.$route.query.dialog_id, {
      initData: window.Telegram.WebApp.initData,
      index: this.$route.query.index
    }).then((response) => {
      const md = new MarkdownIt({
        html: true,      // разрешить HTML в Markdown (обязательно чистить!)
        linkify: true,   // превращать URL в ссылки
